...
public class TacticalRetreatGoal extends Goal {
    ...
    public boolean canContinueToUse() {
        ...
        double distanceToTarget = mob.distanceTo(target);
        double safeDistanceSqr = (retreatDistance + 2.0);

        return distanceToTarget < safeDistanceSqr;
    }
    ...
    public void start() {
        ...
        com.khotyz.buffmobs.BuffMobsMod.LOGGER.debug("{} starting retreat from {}",
    ...
    public void tick() {
        ...
        if (target != null) {
            ...
            if (mob.getNavigation().isDone() || retreatTimer % 5 == 0) {
                Vec3 retreatPos = findRetreatPosition();
                if (retreatPos != null) {
    ...
    private Vec3 findRetreatPosition() {
        ...
        if (mob instanceof PathfinderMob pathfinderMob) {
            for (int distance = 10; distance >= 6; distance -= 1) {
            ...
            Vec3 fallback = DefaultRandomPos.getPosAway(pathfinderMob, 8, 5, targetPos);
            if (fallback != null) {
                return fallback;
            }
        }

        return mobPos.add(awayFromTarget.scale(8));
    }
