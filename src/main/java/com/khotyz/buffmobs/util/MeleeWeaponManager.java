...
public class MeleeWeaponManager {
    ...
    private static void applyEnchantments(ItemStack weapon, double dayMultiplier, Level world) {
        ...
        HolderLookup.RegistryLookup<Enchantment> enchantmentLookup = world.registryAccess().lookupOrThrow(Registries.ENCHANTMENT);

        addEnchantmentIfUnlocked(availableEnchantments, enchantmentLookup,
        ...
        applyRandomEnchantments(weapon, availableEnchantments, maxEnchantments, enchantmentLookup);
    }
    ...
    private static void applyRandomEnchantments(
        ...
        for (int i = 0; i < toApply && !remaining.isEmpty(); i++) {
            ...
            if (selected != null) {
                final EnchantmentTier finalSelected = selected;
                lookup.get(finalSelected.enchantmentKey).ifPresent(enchantmentHolder -> {
                    weapon.enchant(enchantmentHolder, finalSelected.level);
                });
    ...
    private static class EnchantmentTier {
        final ResourceKey<Enchantment> enchantmentKey;
        final int level;
        final double weight;

        EnchantmentTier(ResourceKey<Enchantment> key, int level, double weight) {
